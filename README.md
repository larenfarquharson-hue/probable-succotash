...
.venv/bin/python -m spendtracker.cli import-statement 'samples/*.csv'
.venv/bin/python -m spendtracker.cli report --period 2026-03
.venv/bin/python -m spendtracker.cli advice --period 2026-Q1
.venv/bin/python -m spendtracker.cli serve          # web UI on http://127.0.0.1:5000
```

Then point it at your own statements:

```bash
.venv/bin/python -m spendtracker.cli import-statement ~/Downloads/statement*.csv
```

Or install it so `spendtracker` is on your path:
...
spendtracker serve                        web interface
```
